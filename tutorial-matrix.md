...
```package
neopixelExtended=github:PasAlt/pxt-neopixel-matrix-extension#v2.0.5
```
...
Open the NeoPixel tab on the left side. Then select the first element called **``||neopixelExtended:set strip to..||``** and drag it into the **``on start``** block. 
...
## Pause
To be able to change between two different images we need to keep each image for a second. We will use the **``||basic:pause||``** command for that.

Drag the block **``||basic:pause||``** below the ``show`` block and set it to 1000 miliseconds.
